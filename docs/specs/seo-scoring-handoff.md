...
> **更新**：2026-06-22（评分校准 v3、训练样本版本隔离、≥9 分类门禁、8.7+ 手术式改写）

## 2026-06-22 校准安全修复

- 本地分不是 Semrush 分的线性缩放。现有库曾出现本地 ≥95 的 14 篇中 11 篇 Semrush <9；本地 100 的 5 篇平均 Semrush 8.14。
- `localScoreVersion=2` 起只训练同版本快照；历史无版本快照保留展示，但不进入 v3 模型。
- `rolledBack=true` 的候选快照不再进入训练集。
- 模型生产就绪不再只看 MAE，还要求 Holdout 中有足够的 Semrush ≥9 样本且召回率达标。
- 校准未就绪时，本地进门闸回退到至少 95 分，终检仍以 Semrush RPA ≥9.0 为准。
- 8.7+ 直接走手术式改写，避免 near-miss 文章整篇重写后跌至 7.x。
- 评分实验室提供人工 Semrush 黑盒反推：基线与单变量稿固定提交词/数据库，各重复 3 次；系统计算中位数差值、标准差和置信度。送检必须使用“富文本复制”，否则 Markdown `##` 会被 SWA 当成正文并污染实验。
- 所有 AI 正文结果在落库前必须经过 `validateAndFixSemrushStructure`；该硬闸会删除模型抄入的自动目录，用目录标签恢复被压平的 H2，限制标题长度并补齐 Markdown 块空行。不要只依赖 Prompt 或 RPA 提交时修复。
