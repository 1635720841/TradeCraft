...
export class SeoCheckerService implements OnModuleInit {
  ...
  private buildRewriteSuggestions(result: SeoScore): string[] {
    ...
    pushSection('Readability', details?.readability);
    pushSection('SEO', details?.seo);
    pushSection('Tone', details?.tone);
    pushSection('Originality', details?.originality);
    ...
    if (lines.length === 0) {
      lines.push(
        '[Tone] Reduce template phrasing and AI boilerplate for a more natural voice',
        '[Originality] Add specific examples or unique angles — avoid competitor-parity wording',
        '[Readability] Split long paragraphs, simplify sentences, use active voice',
      );
    }
    ...
    if (
      ...
      lines.unshift(
        `[Readability] Current ~${currentWords} words vs Semrush competitor benchmark ~${competitorWords} words: trim to ${competitorWords}–${competitorWords + 50} words — cut repetition and transition fluff; keep one core point per H2`,
      );
    }
    ...
    if (typeof readability === 'number' && readability < 70) {
      lines.unshift(
        `[Readability] Semrush readability index ${readability}/100 (target ≥70): lower complexity globally — short sentences, simple words, active voice, lists instead of long blocks`,
      );
    }
