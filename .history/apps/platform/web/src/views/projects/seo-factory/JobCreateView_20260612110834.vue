      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="单条关键词" name="single">
          <el-form
            ...
            @submit.prevent
          >
            <el-form-item v-if="showSiteSelector" label="目标站点" prop="siteId">
              <el-select
...
/** 站点管理页未就绪，先隐藏选择器并自动使用项目下首个站点 */
const showSiteSelector = false;
...
function applyDefaultSite() {
  ...
  singleForm.siteId = defaultSite.id;
  batchForm.siteId = defaultSite.id;
}
...
function ensureSiteReady(): boolean {
  if (singleForm.siteId) {
    return true;
  }
  message("当前项目暂无可用站点，请先执行 pnpm db:seed", { type: "warning" });
  return false;
}
